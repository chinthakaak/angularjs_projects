...
{
    $scope.IsNewRecord = 0; //flag value 1 to insert new record and 0 to update existing record 
    
    loadRecords(); // Initially loads all records from the db to view (UI)

    //Function to load all Book records 
    function loadRecords() 
    {
        var promiseGet = crudService.getBooks(); 
        
        //Method Call from the service
        promiseGet.then(
            function (pl) { $scope.Books = pl.data }, 
            function (errorPl)
                                    {
    ...
    $scope.save = function () {
        var Book = {
                name: $scope.name, isbn: $scope.isbn, author: $scope.author, pages: $scope.pages
        };
